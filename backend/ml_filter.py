...
export default function StrategyDashboard() {
  ...
  // Reference for chart to control zoom
  ...
  const runBacktest = async () => {
    try {
      const res = await fetch("http://127.0.0.1:5000/run-backtest", {
        ...
        body: JSON.stringify({ symbols: selectedPairs, timeframes: selectedFrequencies })
      });
    ...
    return (
      <>
        <Line
          ...
          options={{
            ...
            plugins: {
              ...
              title: { display: true, text: selectedLog ? `Trade Log – ${selectedLog}` : "Trade Entry vs Exit" },
              zoom: {
        ...
        <button
          ...
          style={{ marginTop: 10, padding: '6px 12px', background: '#64748b', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}
        >
          Reset Zoom
        </button>
      </>
  ...
  return (
    <div style={{ display: 'flex', gap: '40px', padding: 20, background: '#1e293b', minHeight: '100vh', color: '#f8fafc' }}>
      {/* Left Panel */}
      <div style={{ flex: 1, paddingRight: 20 }}>
        ...
        <div>
          ...
          <input
            ...
            style={{ width: '100%', padding: 8, marginBottom: 10, borderRadius: 4, border: '1px solid #475569', background: '#0f172a', color: '#f8fafc' }}
          />
          <div style={{ maxHeight: 250, overflowY: 'auto', border: '1px solid #475569', padding: 10 }}>
            {currencyPairs.filter(p => p.toLowerCase().includes(search.toLowerCase())).map(p => (
        ...
        <div style={{ marginTop: 20 }}>
          <button
            onClick={runBacktest}
            style={{ background: '#3b82f6', color: '#fff', padding: '10px 20px', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 'bold' }}
          >
            ▶ Run Backtest
          </button>
        </div>
      ...
      <div style={{ flex: 1 }}>
        ...
        {results.length === 0 ? (
          ...
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {results.map((r, i) => (
              <li key={i} style={{ marginBottom: 12, background: '#0f172a', padding: 15, borderRadius: 8, borderLeft: '4px solid #38bdf8' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <strong>{r.symbol} [{r.timeframe}]{r.run_id ? ` – ${r.run_id}` : ''}</strong>{' '}
